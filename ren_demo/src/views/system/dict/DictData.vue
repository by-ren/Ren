<template>
    <el-form ref="searchFormRef" :rules="rules" :inline="true" :model="tableParams" class="demo-form-inline">
        <el-form-item label="查询" prop="searchLike">
            <el-input v-model="tableParams.searchLike" placeholder="字典标签/字典键值" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-table :data="tableData" stripe @sort-change="tableSortHandle">
        <el-table-column prop="dictSort" label="显示顺序" width="140" show-overflow-tooltip sortable="custom"></el-table-column>
        <el-table-column prop="dictLabel" label="字典标签" width="140" show-overflow-tooltip></el-table-column>
        <el-table-column prop="dictValue" label="字典键值" width="200" show-overflow-tooltip></el-table-column>
        <el-table-column prop="dictType" label="字典类型" width="200" show-overflow-tooltip></el-table-column>
        <el-table-column prop="cssClass" label="样式属性" width="200" show-overflow-tooltip></el-table-column>
        <el-table-column prop="listClass" label="表格回显样式" width="200" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column fixed="right" label="操作" min-width="120">
            <template #default="scope">
                <el-button link type="primary" size="small" @click="openModifyDictDataDialog(scope.$index, scope.row)" >修改</el-button>
                <el-button link type="primary" size="small" @click="handleDeleteDictData(scope.$index, scope.row)" >删除</el-button>
            </template>
        </el-table-column>
    </el-table>
    <Pagination
        ...
        @pagination="getList"
    />
    <el-dialog :title="addOrModifyTag == 1 ? '添加字典数据' : '修改字典数据'" v-model="dialogFormAddOrModifyDictData" width="500px">
        <el-form :model="addOrModifyDictDataForm" :rules="addOrModifyDictDataFormRules" ref="addOrModifyDictDataFormRef">
            <el-form-item label="显示顺序" :label-width="addOrModifyDictDataFormLabelWidth" prop="dictSort">
                <el-input v-model="addOrModifyDictDataForm.dictSort" autocomplete="off"></el-input>
            </el-form-item>
            <el-form-item label="字典标签" :label-width="addOrModifyDictDataFormLabelWidth" prop="dictLabel">
                <el-input v-model="addOrModifyDictDataForm.dictLabel" autocomplete="off"></el-input>
            </el-form-item>
            <el-form-item label="字典键值" :label-width="addOrModifyDictDataFormLabelWidth" prop="dictValue">
            ...
            <el-form-item label="表格回显样式" :label-width="addOrModifyDictDataFormLabelWidth" prop="listClass">
                <el-input v-model="addOrModifyDictDataForm.listClass" autocomplete="off"></el-input>
            </el-form-item>
            <el-form-item label="是否默认" :label-width="addOrModifyDictDataFormLabelWidth" prop="isDefault">
                <el-radio-group v-model="addOrModifyDictDataForm.isDefault" size="large" fill="#6cf">
                    <el-radio-button label="是" :value="1" />
                    <el-radio-button label="否" :value="0" />
                </el-radio-group>
            </el-form-item>
            <el-form-item label="备注" :label-width="addOrModifyDictDataFormLabelWidth" prop="remark">
        ...
        <template #footer>
            <span class="dialog-footer">
                ...
                <el-button type="primary" @click="addOrModifyDictDataConfirm(addOrModifyDictDataFormRef)">
                    确 定
                </el-button>
            </span>
        </template>
...
<script setup lang="ts" name="dictData">
    ...
    const addOrModifyDictDataFormRules = reactive<FormRules>({
        ...
        dictLabel:[
            { required: true, message: '请填写字典标签', trigger: 'blur' }
        ],
        dictValue:[
