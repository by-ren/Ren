<template>
    ...
    <el-form ref="searchFormRef" :rules="rules" :inline="true" :model="tableParams" class="demo-form-inline">
        <el-form-item label="查询" prop="searchLike">
            <el-input v-model="tableParams.searchLike" placeholder="菜单名称/路由名称/权限" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    >
      ...
      <el-table-column fixed="right" label="操作" min-width="120">
            <template #default="scope">
                <el-button link type="primary" size="small" @click="openModifyMenuDialog(scope.$index, scope.row)">修改</el-button>
                <el-button link type="primary" size="small" @click="openSubAddOrModifyMenuDialog(scope.$index, scope.row)">添加子级菜单</el-button>
                <el-button link type="primary" size="small" @click="handleDeleteMenu(scope.$index, scope.row)">删除</el-button>
            </template>
        </el-table-column>
    </el-table>
    <el-dialog :title="addOrModifyTag == 1 ? '添加菜单' : '修改菜单'" v-model="dialogFormAddOrModifyMenu" width="700px">
        <el-form :model="addOrModifyMenuForm" :rules="addOrModifyMenuFormRules" ref="addOrModifyMenuFormRef" :inline="true">
            ...
            <el-form-item label="菜单类型" :label-width="addOrModifyMenuFormLabelWidth" prop="menuType" class="bigLine" >
            ...
            <el-form-item label="菜单图标" :label-width="addOrModifyMenuFormLabelWidth" prop="icon" class="bigLine" v-if="addOrModifyMenuForm.menuType=='M' || addOrModifyMenuForm.menuType=='C'">
                <IconPicker v-model="addOrModifyMenuForm.icon"/>
            </el-form-item>
            <el-form-item label="菜单名称" :label-width="addOrModifyMenuFormLabelWidth" prop="menuName" class="smallLine" >
            ...
            <el-form-item label="是否缓存" :label-width="addOrModifyMenuFormLabelWidth" prop="isCache" class="smallLine" v-if="addOrModifyMenuForm.menuType=='C'">
                <el-radio-group v-model="addOrModifyMenuForm.isCache" size="large" fill="#6cf">
                    <el-radio-button label="否" :value="0" />
                    <el-radio-button label="是" :value="1" />
                </el-radio-group>
            </el-form-item>
            <el-form-item label="是否隐藏" :label-width="addOrModifyMenuFormLabelWidth" prop="isVisible" class="smallLine" v-if="addOrModifyMenuForm.menuType=='M' || addOrModifyMenuForm.menuType=='C'">
                <el-radio-group v-model="addOrModifyMenuForm.isVisible" size="large" fill="#6cf">
                    <el-radio-button label="否" :value="0" />
                    <el-radio-button label="是" :value="1" />
                </el-radio-group>
            </el-form-item>
            <el-form-item label="是否停用" :label-width="addOrModifyMenuFormLabelWidth" prop="isStop" class="smallLine" >
                <el-radio-group v-model="addOrModifyMenuForm.isStop" size="large" fill="#6cf">
                    <el-radio-button label="否" :value="0" />
                    <el-radio-button label="是" :value="1" />
                </el-radio-group>
            </el-form-item>
            <el-form-item label="备注" :label-width="addOrModifyMenuFormLabelWidth" prop="remark" class="bigLine" >
